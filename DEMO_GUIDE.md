...
[onboard] [DRY RUN] Acting on changes...
  ...
  [new] petclinicmbtest
    $ python3 core/provision_detectors.py --environment petclinicmbtest
      [dry-run] skipped
...
- 13 SignalFlow detectors (error rate + latency + request rate per service)
...
- *"Teardown is one command: `python3 onboard.py --teardown --environment <env>`. Removes all detectors, cron jobs, and dashboard entries."*
