...
> "Today I want to show you something we built on top of Splunk Observability APM — a behavioral anomaly detection framework that catches the class of failures that standard metric-based alerting completely misses.
>
...
>
> Tier 1 is AutoDetect — already built into Splunk, always on, covers error rate, latency, and request rate. We consume its incidents as inputs.
>
> Tier 2 is trace path drift. We learn the structural fingerprint of every service — which services always appear in traces, what call paths are normal — and we watch for deviations every five minutes. Missing service, unknown path, unexpected extra hops.
>
> Tier 3 is error signature drift. We learn every known exception type per service. The first time a new one appears, we fire immediately — before you'd have any rate to threshold against.
>
...
>
> Learn: we sample live traces from Splunk APM and build structural fingerprints and error baselines from real traffic. This runs once on onboarding and then nightly to keep up with normal evolution.
>
> Watch — every five minutes: we compare incoming traces against the baseline. Any deviation emits a custom event to Splunk.
>
> Correlate — also every five minutes: we join Tier 1, 2, and 3 events by service and time window. This is where MULTI_TIER is determined. We also pull in deployment events from the CI/CD hook here.
>
...
>
> First-occurrence detection: this is unique. We fire the moment a new error signature or new trace path appears — before any rate exists to compare against. You catch the first occurrence, not the hundredth.
>
...
> "So the ask here isn't 'can we run this in one environment.' The ask is: should this become a native capability inside Splunk Observability Cloud?
>
