#!/usr/bin/env bash
# demo-latency.sh — Demo 7: LATENCY_ANOMALY
#
# Story: visits-service is running normally. The OTel processor learns its
# latency baseline over ~30 traces (~2 min of traffic). We then inject 3s of
# network delay into the pod via tc-netem (run from the k3d node using nsenter),
# causing span durations to spike. The processor detects a z-score >3 and emits
# "latency anomaly detected". Claude triages: DEGRADED, visits-service slowdown.
#
# Usage:
#   ./demo/demo-latency.sh              # full demo (inject → watch → triage)
#   ./demo/demo-latency.sh --inject     # just start the slowdown
#   ./demo/demo-latency.sh --stop       # remove tc delay and restore normal latency
...
TARGET="visits-service"
DELAY_MS=3000   # 3s added to every network call — easily exceeds 3σ above ~50ms baseline
...
_get_pod_netns_pid() {
  ...
  # Find first host PID whose cgroup contains this container ID
...
# ── Step 2: Confirm learn phase ────────────────────────────────────────────────
echo "[2] Learn phase check:"
echo "    The OTel processor needs 30 traces for $TARGET to build a latency baseline."
echo "    With petclinic loadgen running, this takes ~2 minutes."
echo "    Current uptime of otelcol-fingerprint pods:"
$K "kubectl get pods -l app=otelcol-fingerprint -o custom-columns='NAME:.metadata.name,AGE:.metadata.creationTimestamp' 2>/dev/null" \
  2>/dev/null | grep -v '▀\|█\|▄' || true
echo ""
...
if [ -z "$NS_PID" ]; then
  ...
  echo "  Ensure crictl is available on the k3d node and the pod is running."
  exit 1
fi
...
echo "    Delay active. visits-service spans will spike from ~3ms baseline to ~753ms."
...
if [ "$INJECT" = "true" ]; then
  ...
  echo ""
  echo "To stop: ./demo/demo-latency.sh --stop"
  exit 0
fi
